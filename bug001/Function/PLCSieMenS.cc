 ...
 * @Last Modified time: 2020-04-25 00:02:31
 */

#include "../headers/PLCSieMenS.h"
...
bool CPLCSieMenS::sendEnv()
{
    return true;
}

bool CPLCSieMenS::getEnv()
{
    return true;
}

//控制电路开关,一共有四路开关
void CPLCSieMenS::control(std::string action, std::vector<int> power)
{
...
CPLCSieMenS* CPLCSieMenS::instance()
{
    static CPLCSieMenS* _instance = NULL;
    if(_instance == NULL){
        _instance = new CPLCSieMenS();
    }

    return _instance;  
}

CPLCSieMenS::CPLCSieMenS():t(&CPLCSieMenS::run, this)
{
